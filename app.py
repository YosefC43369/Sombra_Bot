...
async def chat_id_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/id -- ตอบกลับ Chat ID ของกลุ่มนี้ (และ User ID ผู้ส่ง ถ้ามี Reply
    ก็แสดง User ID ของข้อความที่ Reply ด้วย) -- เดิมถูก register ไว้ใน main()
    แต่ไม่เคยมี function นี้อยู่จริงในไฟล์ ทำให้ main() พังทันทีตอน build
    handlers (NameError); เพิ่ม implementation ขั้นต่ำให้ใช้งานได้จริง"""
    chat = update.effective_chat
    lines = [f"Chat ID: {chat.id}"]
    if update.message.reply_to_message:
        target = update.message.reply_to_message.from_user
        lines.append(f"User ID (Reply): {target.id}")
    else:
        lines.append(f"User ID: {update.effective_user.id}")
...
#
# Phase 5 adds /bbprogram, /bbauth, /bbscope, /bbcheck (below), so a
# program_id/authorization can now be obtained entirely through
# Telegram before /bbfinding new is used -- see those handlers for the
# same admin-gating rationale applied to Program/Authorization/Scope
# mutations.
...
# Built on expense.py (data layer) / expense_report.py (Thai formatting),
# following the same conventions as the wallet and debt commands above:
# subcommand dispatch on args[0] (/bbprogram style), money parsed exactly
# once via ex.parse_amount_to_satang() and never touched as a float, and
# the existing is_admin() for the one chat-wide view.
#
# Deliberately command-based rather than InlineKeyboard/CallbackQuery:
# this repo registers no CallbackQueryHandler and uses no reply_markup
# anywhere (debt_ledger.debtor_ref() was written in anticipation of one
# but nothing ever wired it up), so there is no existing callback
# routing to reuse -- introducing a bot-wide callback router just for
# this feature would be a far larger and riskier change than the
# feature warrants. Delete therefore uses a typed two-step confirm.
#
# Expenses NEVER move a wallet balance and NEVER settle a debt entry --
# see expense.py's module docstring for why.


def _expense_period_or_error(args):
    """Shared period parsing for /expenses, /expense_report and
    /expense_category. Returns (period_dict, None) or (None, error_text)."""
    period = ex.parse_period(args)
    if period is None:
        return None, (
            "❌ ช่วงเวลาไม่ถูกต้อง\n"
            "ใช้ได้: today | week | month | YYYY-MM | YYYY-MM-DD | "
            "YYYY-MM-DD YYYY-MM-DD"
        )
    return period, None


def _split_page_arg(args):
    """Pulls a trailing 'page N' off an argument list, so period words
    and pagination can be combined ('/expenses week page 2'). Returns
    (remaining_args, page)."""
    args = list(args or [])
    page = 1
    if len(args) >= 2 and args[-2].lower() == "page" and args[-1].isdigit():
...
async def cmd_expense(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/expense <จำนวนเงิน> <หมวดหมู่> [รายละเอียด...] -- บันทึกรายจ่าย 1 รายการ
    ตัวอย่าง: /expense 150 Food ข้าวมันไก่ / /expense 120 อาหาร มื้อกลางวัน

    หมายเหตุ: การบันทึกรายจ่าย ไม่ตัดยอดเงินในกระเป๋า (/wallet) และไม่ปิดยอดหนี้
    (/debt) -- เป็นการจดบันทึกค่าใช้จ่ายเท่านั้น"""
    args = context.args or []
    chat_id = update.effective_chat.id
    user = update.effective_user
    ...
    if len(args) < 2:
        return await _reply_chunked(update, (
            "ใช้งาน: /expense <จำนวนเงิน> <หมวดหมู่> [รายละเอียด...]\n"
            "ตัวอย่าง: /expense 150 Food ข้าวมันไก่\n\n"
            + exr.category_list_text()
        ))

    amount_satang = ex.parse_amount_to_satang(args[0])
    if amount_satang is None:
        return await update.message.reply_text(exr.deny_text("INVALID_AMOUNT"))

    category = ex.normalize_category(args[1])
    if category is None:
        return await _reply_chunked(
            update, exr.deny_text("INVALID_CATEGORY") + "\n\n" + exr.category_list_text()
        )

    description = " ".join(args[2:])
    result = ex.add_expense(chat_id, user.id, amount_satang, category, description)
    if not result.ok:
        return await update.message.reply_text(exr.deny_text(result.reason))
    await update.message.reply_text(exr.format_expense_added(result.data))


async def cmd_expenses(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/expenses [today|week|month|YYYY-MM|YYYY-MM-DD [YYYY-MM-DD]] [page N]
    -- ดูรายจ่ายของตัวเอง (ไม่ระบุช่วงเวลา = เดือนนี้)"""
    args, page = _split_page_arg(context.args)
    period, error = _expense_period_or_error(args)
    if error:
        return await update.message.reply_text(error)

    chat_id = update.effective_chat.id
    user = update.effective_user
    page_data = ex.list_expenses(chat_id, user.id, date_from=period["date_from"],
                                 date_to=period["date_to"], page=page)
    await _reply_chunked(
        update, exr.format_expense_list(page_data, title=f"รายจ่าย — {period['label']}")
    )


async def cmd_expense_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/expense_edit <id> amount <จำนวนเงิน>
    /expense_edit <id> category <หมวดหมู่>
    /expense_edit <id> desc <รายละเอียด...>
    /expense_edit <id> date <YYYY-MM-DD>
    -- แก้ไขได้เฉพาะรายการของตัวเองเท่านั้น"""
    args = context.args or []
    ...
    kwargs = {}
    if field_name in ("amount", "จำนวน", "จำนวนเงิน"):
        amount_satang = ex.parse_amount_to_satang(raw_value)
        if amount_satang is None:
            return await update.message.reply_text(exr.deny_text("INVALID_AMOUNT"))
        kwargs["amount_satang"] = amount_satang
    elif field_name in ("category", "หมวด", "หมวดหมู่"):
        category = ex.normalize_category(raw_value)
        if category is None:
            return await _reply_chunked(
                update, exr.deny_text("INVALID_CATEGORY") + "\n\n" + exr.category_list_text()
            )
        kwargs["category"] = category
    elif field_name in ("desc", "description", "รายละเอียด"):
        kwargs["description"] = raw_value
    elif field_name in ("date", "วันที่"):
        kwargs["expense_date"] = raw_value.strip()
    else:
        return await update.message.reply_text(
            "❌ แก้ไขได้เฉพาะ: amount / category / desc / date"
        )

    result = ex.update_expense(chat_id, expense_id, user.id, **kwargs)
    if not result.ok:
        return await update.message.reply_text(exr.deny_text(result.reason))
    await update.message.reply_text(exr.format_expense_updated(result.data["expense"]))


async def cmd_expense_delete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/expense_delete <id> -- แสดงรายการแล้วขอให้ยืนยัน
    /expense_delete <id> confirm -- ลบจริง (ลบได้เฉพาะรายการของตัวเอง)"""
    args = context.args or []
    if not args or not args[0].isdigit():
        return await update.message.reply_text("ใช้งาน: /expense_delete <id> [confirm]")
    expense_id = int(args[0])
    chat_id = update.effective_chat.id
    user = update.effective_user
    confirmed = len(args) >= 2 and args[1].lower() in ("confirm", "ยืนยัน")

    if not confirmed:
        # Ownership-scoped read: someone else's row reads back as None, so
        # the confirm prompt can never disclose another member's expense.
        row = ex.get_expense(chat_id, expense_id, user_id=user.id)
        if not row:
            return await update.message.reply_text(exr.deny_text("NOT_FOUND"))
        return await _reply_chunked(update, exr.format_delete_confirm(row))

    result = ex.delete_expense(chat_id, expense_id, user.id)
    if not result.ok:
        return await update.message.reply_text(exr.deny_text(result.reason))
    await update.message.reply_text(exr.format_expense_deleted(result.data))


async def cmd_expense_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/expense_report [today|week|month|YYYY-MM|YYYY-MM-DD [YYYY-MM-DD]]
    -- สรุปรายจ่ายของตัวเองแยกตามหมวดหมู่ (ไม่ระบุ = เดือนนี้)
    /expense_report all [ช่วงเวลา] -- Admin: สรุปรายจ่ายทั้งกลุ่มแยกตามผู้ใช้"""
    args = list(context.args or [])
    chat_id = update.effective_chat.id
    user = update.effective_user
    ...
    if args and args[0].lower() == "all":
        if not await is_admin(update, context):
            return await update.message.reply_text("❌ คำสั่งนี้ใช้ได้เฉพาะ Admin")
        period, error = _expense_period_or_error(args[1:])
        if error:
            return await update.message.reply_text(error)
        summary = ex.summarize_all_users_admin(chat_id, date_from=period["date_from"],
                                               date_to=period["date_to"])
        return await _reply_chunked(
            update, exr.format_admin_summary(summary, period_label=period["label"])
        )

    period, error = _expense_period_or_error(args)
    if error:
        return await update.message.reply_text(error)
    summary = ex.summarize_by_category(chat_id, user.id, date_from=period["date_from"],
                                       date_to=period["date_to"])
    await _reply_chunked(
        update, exr.format_category_summary(summary, period_label=period["label"])
    )


async def cmd_expense_category(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/expense_category <หมวดหมู่> [ช่วงเวลา] [page N] -- ดูรายจ่ายของตัวเอง
    เฉพาะหมวดหมู่หนึ่ง พร้อมยอดรวมและจำนวนรายการ"""
    args, page = _split_page_arg(context.args)
    if not args:
        return await _reply_chunked(update, (
            "ใช้งาน: /expense_category <หมวดหมู่> [ช่วงเวลา]\n"
            "ตัวอย่าง: /expense_category Food month\n\n" + exr.category_list_text()
        ))

    category = ex.normalize_category(args[0])
    if category is None:
        return await _reply_chunked(
            update, exr.deny_text("INVALID_CATEGORY") + "\n\n" + exr.category_list_text()
        )
    ...
    user = update.effective_user
    page_data = ex.list_expenses(chat_id, user.id, date_from=period["date_from"],
                                 date_to=period["date_to"], category=category, page=page)
    summary = ex.summarize_by_category(chat_id, user.id, date_from=period["date_from"],
                                       date_to=period["date_to"])
    summary_row = next(
        (c for c in summary["by_category"] if c["category"] == category),
        {"category": category, "count": 0, "total_satang": 0},
    )
    await _reply_chunked(
        update,
        exr.format_single_category_report(page_data, summary_row,
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type in (ChatType.GROUP, ChatType.SUPERGROUP):
        wt.remember_user(chat.id, user.id, user.username, user.full_name)
...
def main():
    ...
    app.add_handler(CommandHandler("expense_edit", cmd_expense_edit))
    app.add_handler(CommandHandler("expense_delete", cmd_expense_delete))
    app.add_handler(CommandHandler("expense_report", cmd_expense_report))
    app.add_handler(CommandHandler("expense_category", cmd_expense_category))
