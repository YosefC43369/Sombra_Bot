...
def get_entry_in_conn(conn, entry_id: int) -> Optional[dict]:
    """Same read as get_entry(), but against a connection the caller
    already has open (e.g. wallet.py's pay_debt_with_wallet(), running
    inside its own BEGIN IMMEDIATE). Lets a fresh, re-checked read of
    this entry participate in the caller's own atomic commit/rollback
...
def _mark_entry_paid_in_conn(conn, entry: dict, actor_user_id: int) -> dict:
    """Same UPDATE as mark_entry_paid(), but against the caller's own
    open connection so it commits atomically together with the
    caller's other writes (e.g. wallet.py's wallet debit) in a single
    transaction -- see pay_debt_with_wallet() (Requirement #8: wallet
    deduction + debt update + ledger transaction must be atomic).
    Caller must have already validated entry['status'] on a fresh read
    from this same connection (pay_debt_with_wallet() does, via
    get_entry_in_conn() above) -- this function does not re-check
    ALREADY_PAID itself. Audit logging is deliberately left to the
    caller, done after commit, matching every other function in this
    codebase (write_audit_log opens its own connection, so calling it
    while still holding this transaction's write lock would only add
    unnecessary lock contention for no atomicity benefit -- the audit
    row is a record of what happened, not part of the invariant that
    must be atomic)."""
    now = _now_utc_epoch()
    conn.execute(
        "UPDATE debt_entries SET status=?, paid_at=?, paid_by=? WHERE entry_id=?",
        (EntryStatus.PAID.value, now, actor_user_id, entry["entry_id"]),
    )
    paid_entry = dict(entry)
    paid_entry["status"] = EntryStatus.PAID.value
